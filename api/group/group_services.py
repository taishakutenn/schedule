...
class GroupService:
    ...
    async def _get_group_by_advisor(self, advisor_id: int, request: Request, db) -> ShowGroupListWithHATEOAS:
        async with db as session:
            async with session.begin():
                group_dal = GroupDAL(session)
                teacher_dal = TeacherDAL(session)
                try:
                    ...
                    groups = await group_dal.get_groups_by_advisor_id(advisor_id)
                    if not groups:
                        raise HTTPException(status_code=404, detail=f"Группы преподавателя с id {advisor_id} не найдены")
                    ...
                    collection_links = {
                        "self": f'{api_base_url}/groups/search/by_advisor/{advisor_id}',
                        "create": f'{api_base_url}/groups/create',
                        "teacher": f'{api_base_url}/teachers/search/by_id/{advisor_id}'
                    }
                    ...
                    logger.error(f"Неожиданная ошибка при получении групп по преподавателю {advisor_id}: {e}", exc_info=True)
                    raise HTTPException(status_code=500, detail="Внутренняя ошибка сервера при получении групп по преподавателю.")
    ...
    async def _get_subjects_by_group_and_teacher_id(self, group_name: str, teacher_id: int, request: Request, db) -> ShowSubjectList:
        ...
        async with db as session:
            async with session.begin():
                ...
                try:
                    ...
                    # Получаем предметы с лекциями
                    subjects = await group_dal.get_subjects_by_group_and_teacher_id(group_name, teacher_id)
